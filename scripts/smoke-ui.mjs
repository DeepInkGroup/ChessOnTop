...
try {
  ...
  assert.equal(await page.locator(".board-wrap marker").first().getAttribute("markerUnits"), "userSpaceOnUse");
  assert.equal(await page.locator(".board-wrap .board-arrow").first().evaluate((arrow) => getComputedStyle(arrow).strokeLinecap), "butt");
  ...
  await page.getByLabel("Book cover image").setInputFiles(resolve(root, "public/pieces/wQ.svg"));
  await page.locator(".admin-book-form .image-upload-preview").waitFor();
  const bookEditor = page.getByRole("textbox", { name: "Book content" });
  await bookEditor.fill("Chapter 1 — Look first\n\nBefore every move, check your king and your opponent's threats.");
  ...
  await page.getByLabel("Article feature image").setInputFiles(resolve(root, "public/pieces/wQ.svg"));
  await page.locator(".admin-article-form .image-upload-preview").waitFor();
  const articleEditor = page.getByRole("textbox", { name: "Article content" });
  await articleEditor.fill("Start with your opponent's last move.\n\nThen scan checks, captures, and threats.");
  ...
  const articleReader = page.getByRole("dialog", { name: /Reading Three questions/ });
  ...
  assert.equal(await page.locator(".book-cover-image").count(), 1);
  await page.getByRole("button", { name: "Read book" }).click();
  const bookReader = page.getByRole("dialog", { name: /Reading Winning Chess Habits/ });
